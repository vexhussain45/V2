...
// List participants who requested to join the group
// ---------------------------------------------------------------------
const joinRequestsCommand = {
...
cmd(joinRequestsCommand, async (client, message, args, { from, q, reply, isGroup }) => {
  if (!isGroup) {
    return reply("This command can only be used in a group chat.");
  }
  try {
    console.log(`Attempting to fetch pending requests for group: ${from}`);
    const pendingRequests = await client.groupRequestParticipantsList(from);
    console.log(pendingRequests);
    if (pendingRequests.length > 0) {
      let text = "Pending Requests to Join the Group:\n";
      let mentions = [];
      pendingRequests.forEach(request => {
        const participantJid = request.jid;
        text += `😻 @${participantJid.split('@')[0]}\n`;
        mentions.push(participantJid);
      });
      const response = { text, mentions };
...
// ---------------------------------------------------------------------
// ALL REQUESTS COMMAND
// Approve or reject all pending join requests
// ---------------------------------------------------------------------
const allRequestsCommand = {
  pattern: "allreq",
...
cmd(allRequestsCommand, async (client, message, args, { from, reply, isGroup }) => {
  if (!isGroup) {
    return reply("This command can only be used in a group chat.");
  }
  const action = message.body.includes("approve") ? "approve" : "reject";
  try {
    const pendingRequests = await client.groupRequestParticipantsList(from);
    if (pendingRequests.length === 0) {
    ...
    const response = { text, mentions };
    await client.sendMessage(from, response);
    const updateResponse = await client.groupRequestParticipantsUpdate(from, participantJids, action);
...
// ---------------------------------------------------------------------
// DISAPPEARING MESSAGES TOGGLE COMMAND
// Turn on/off disappearing messages for the chat
// ---------------------------------------------------------------------
const disappearCommand = {
  pattern: "disappear",
  react: "🌪️",
  alias: ['dm'],
...
cmd(disappearCommand, async (client, message, args, { from, isGroup, isAdmins, args: commandArgs }) => {
  if (!isGroup) {
    await client.sendMessage(from, { text: "This command can only be used in groups." });
    return;
  }
  if (!isAdmins) {
    await client.sendMessage(from, { text: "Only admins can turn on/off disappearing messages." });
    return;
  }
  const action = commandArgs[0];
  if (action === 'on') {
    const durationInput = commandArgs[1];
    let durationSeconds;
    ...
    await client.sendMessage(from, { disappearingMessagesInChat: durationSeconds });
    await client.sendMessage(from, { text: `Disappearing messages are now ON for ${durationInput}.` });
  } else if (action === 'off') {
    await client.sendMessage(from, { disappearingMessagesInChat: false });
    await client.sendMessage(from, { text: "Disappearing messages are now OFF." });
  } else {
    await client.sendMessage(from, { text: "Please use `!disappear on <duration>` or `!disappear off`." });
  }
});

// ---------------------------------------------------------------------
// SEND DISAPPEARING MESSAGE COMMAND
// Send a message that will disappear after a set duration
// ---------------------------------------------------------------------
const sendDmCommand = {
  pattern: "senddm",
  react: "🌪️",
...
cmd(sendDmCommand, async (client, message, args, { from, isGroup, args: commandArgs }) => {
  if (!isGroup) {
    await client.sendMessage(from, { text: "This command can only be used in groups." });
    return;
  }
  if (!commandArgs.length) {
    await client.sendMessage(from, { text: "Please provide a message to send." });
    return;
  }
  const dmMessage = commandArgs.join(" ");
  const textMessage = { text: dmMessage };
  const options = { ephemeralExpiration: 604800 }; // 7 days in seconds
  await client.sendMessage(from, textMessage, options);
});

// ---------------------------------------------------------------------
// MUTE GROUP COMMAND
// Set group to announcement mode (only admins can chat)
// ---------------------------------------------------------------------
const muteCommand = {
  pattern: "mute",
  react: "🔇",
  alias: ["close", "f_mute"],
  desc: "Change group settings so that only admins can send messages.",
  category: "group",
  use: ".mute",
  filename: __filename
};

cmd(muteCommand, async (client, message, args, context) => {
  const { from, quoted, isGroup, pushname, isAdmins, isBotAdmins, isDev, reply } = context;
  try {
    const mreply = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(mreply.only_gp);
    if (!isAdmins && !isDev) return reply(mreply.you_adm);
    if (!isBotAdmins) return reply(mreply.give_adm);
    await client.groupSettingUpdate(from, "announcement");
    const closeMessage = { text: `*Yooh Guys Iam Tired 😓, Its Time to rest, Group Chat closed by Admin ${pushname}* 🔇` };
    await client.sendMessage(from, closeMessage, { quoted });
  } catch (error) {
...
// Set group so that everyone can send messages
// ---------------------------------------------------------------------
const unmuteCommand = {
  pattern: "unmute",
  react: "🔇",
  alias: ["open", "f_unmute"],
  desc: "Change group settings so that all members can send messages.",
  category: "group",
  use: ".unmute",
  filename: __filename
};

cmd(unmuteCommand, async (client, message, args, context) => {
  const { from, quoted, isGroup, pushname, isAdmins, isBotAdmins, isDev, reply } = context;
  try {
    const mreply = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(mreply.only_gp);
    ...
    const openMessage = { text: `*At Last, Group Chat Opened by Admin ${pushname}* 🔇` };
    await client.sendMessage(from, openMessage, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply(`❌ *Error Occurred!!*\n\n${error}`);
...
// ---------------------------------------------------------------------
// LEAVE GROUP COMMAND
// Send a goodbye message and leave the group
// ---------------------------------------------------------------------
const leaveGroupCommand = {
  pattern: "leave",
  react: "🔓",
...
cmd(leaveGroupCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isDev, quoted, reply } = context;
  try {
    ...
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
    await client.sendMessage(from, { text: "*Good Bye All* 👋🏻" }, { quoted });
...
cmd(updateGroupNameCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, q, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
    if (!isBotAdmins) return reply(replyMsg.give_adm);
    if (!q) return reply("*Please write the new Group Subject* 🖊️");
    await client.groupUpdateSubject(from, q);
    await client.sendMessage(from, { text: "✔️ *Group name Updated*" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply(`❌ *Error Occurred!!*\n\n${error}`);
...
// ---------------------------------------------------------------------
const updateGroupDescriptionCommand = {
  pattern: "updategdesc",
  react: "🔓",
  alias: ["upgdesc", "gdesc"],
...
cmd(updateGroupDescriptionCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, q, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    ...
    await client.sendMessage(from, { text: "✔️ *Group Description Updated*" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply(`❌ *Error Occurred!!*\n\n${error}`);
...
cmd(joinGroupCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isDev, quoted, q, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isAdmins && !isDev) return reply(replyMsg.own_cmd);
    if (!q) return reply("*Please write the Group Link* 🖇️");
    const inviteLink = args[0];
    const inviteCode = inviteLink.split("https://chat.whatsapp.com/")[1];
    await client.groupAcceptInvite(inviteCode);
    await client.sendMessage(from, { text: "✔️ *Successfully Joined*" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply(`❌ *Error Occurred!!*\n\n${error}`);
...
// Retrieve and display the group invite link
// ---------------------------------------------------------------------
const getGroupInviteCommand = {
  pattern: "invite",
  react: "🖇️",
...
cmd(getGroupInviteCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
    ...
    const inviteText = `🖇️ *Group Link*\n\nhttps://chat.whatsapp.com/${inviteCode}`;
    await client.sendMessage(from, { text: inviteText }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply(`❌ *Error Occurred!!*\n\n${error}`);
...
cmd(revokeCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    ...
    await client.sendMessage(from, { text: "*Group link Reseted* ⛔" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply("❌ *Error Occurred!!*\n\n" + error);
...
cmd(kickCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
    if (!isBotAdmins) return reply(replyMsg.give_adm);
    let targetJid = quoted.mentionedJid
      ? quoted.mentionedJid[0]
      : quoted.msg.contextInfo.participant || false;
    if (!targetJid) return reply("*Couldn't find any user in context* ❌");
    await client.groupParticipantsUpdate(from, [targetJid], "remove");
    await client.sendMessage(from, { text: "*Successfully removed*  ✔️" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply("❌ *Error Occurred!!*\n\n" + error);
...
const promoteCommand = {
  ...
  react: "🥏",
  alias: ["addadmin"],
  desc: "Add a participant as an admin",
  category: "group",
  use: ".promote",
  filename: __filename,
};

cmd(promoteCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply, participants } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
    ...
    if (currentAdmins.includes(targetJid)) {
      return reply("❗ *User Already an Admin*  ✔️");
    }
    await client.groupParticipantsUpdate(from, [targetJid], "promote");
    await client.sendMessage(from, { text: "*User promoted as an Admin*  ✔️" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply("❌ *Error Occurred!!*\n\n" + error);
...
// Remove admin rights from a participant
// ---------------------------------------------------------------------
const demoteCommand = {
  pattern: "demote",
  react: "🥏",
...
cmd(demoteCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply, participants } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    ...
    await client.groupParticipantsUpdate(from, [targetJid], "demote");
    await client.sendMessage(from, { text: "*User no longer an Admin*  ✔️" }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply("❌ *Error Occurred!!*\n\n" + error);
...
// ---------------------------------------------------------------------
// TAG ALL COMMAND
// Mention every participant in the group
// ---------------------------------------------------------------------
const tagAllCommand = {
  pattern: "tagall",
  react: "🔊",
...
cmd(tagAllCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply, participants } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
    if (!isBotAdmins) return reply(replyMsg.give_adm);
    let tagText = "💱 *HI ALL ! GIVE YOUR ATTENTION PLEASE*\n\n";
    participants.forEach((participant) => {
      tagText += `> ᴅᴇᴀʀ ☣️ @${participant.id.split("@")[0]}\n`;
    });
...
// ---------------------------------------------------------------------
// HIDETAG COMMAND
// Send a message with hidden tags (only notifies mentioned users)
// ---------------------------------------------------------------------
const hidetagCommand = {
  pattern: "hidetag",
  react: "🔊",
  alias: ["tag", "f_tag"],
  desc: "Send a message with hidden tags",
  category: "group",
  use: ".tag Hi",
  filename: __filename,
};

cmd(hidetagCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply, participants, q } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    if (!isAdmins && !isDev) return reply(replyMsg.you_adm);
...
// ---------------------------------------------------------------------
// TAG GROUP COMMAND
// Use a quoted message as content to tag all members in a specified group jid
// ---------------------------------------------------------------------
const tagGroupCommand = {
  pattern: "taggp",
  react: "🔊",
  alias: ["tggp", "f_taggp"],
  desc: "Tag all members using a quoted message",
  category: "group",
  use: ".tag Hi",
  filename: __filename,
};

cmd(tagGroupCommand, async (client, message, args, context) => {
  const { from, quoted, reply, participants, q } = context;
  try {
    if (!quoted) return reply("*Please mention a message* ℹ️");
    if (!q) return reply("*Please add a Group Jid* ℹ️");
    let quotedText = "" + quoted.msg;
...
// ---------------------------------------------------------------------
// GROUP INFO COMMAND
// Get group details including subject, ID, participant count, creator, description, and profile picture.
// ---------------------------------------------------------------------
...
cmd(groupInfoCommand, async (client, message, args, context) => {
  const { from, isGroup, isAdmins, isBotAdmins, isDev, quoted, reply } = context;
  try {
    const replyMsg = (await fetchJson("https://raw.githubusercontent.com/SILENTLOVER40/SILENT-SOBX-MD-DATA/refs/heads/main/DATABASE/mreply.json")).replyMsg;
    if (!isGroup) return reply(replyMsg.only_gp);
    ...
    const infoText =
      "\n*" + groupData.subject + "*\n\n🐉 *Group Jid* - " + groupData.id +
      "\n\n📬 *Participant Count* - " + groupData.size +
      "\n\n👤 *Group Creator* - " + groupData.owner +
      "\n\n📃 *Group Description* - " + groupData.desc + "\n\n";
    await client.sendMessage(from, { image: { url: profilePicUrl }, caption: infoText + config.FOOTER }, { quoted });
  } catch (error) {
    await client.sendMessage(from, { react: { text: "❌", key: quoted.key } });
    console.error(error);
    reply("❌ *Error Occurred!!*\n\n" + error);
