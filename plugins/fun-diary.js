...
cmd({
    ...
    use: '.diarydelete <passphrase> <note number>',
    react: "🗑️",
    ...
    try {
        const passphrase = body.split(' ')[1]; // Extract the passphrase
        const noteNumber = parseInt(body.split(' ')[2]); // Extract the note number

        if (!passphrase) {
            return reply("⚠️ Please provide your passphrase. Example: `.diarydelete your_passphrase 1`");
        }
        ...
        if (isNaN(noteNumber) || noteNumber < 1) {
            return reply("Please provide a valid note number. Example: `.diarydelete your_passphrase 1`");
        }
...
cmd({
    ...
    try {
        ...
        if (!passphrase) {
            return reply("⚠️ Please provide your passphrase. Example: `.diarydeleteall your_passphrase`");
        }
